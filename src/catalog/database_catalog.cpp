...
std::unique_ptr<optimizer::TableStats> DatabaseCatalog::GetTableStats(
  ...
  for (const auto &slot : index_results) {
    ...
    NOISEPAGE_ASSERT(nullfrac != nullptr, "nullfrac shouldn't be NULL.");
    ...
    NOISEPAGE_ASSERT(distinct != nullptr, "distinct shouldn't be NULL.");
    ...
    NOISEPAGE_ASSERT(num_rows != nullptr, "num_rows shouldn't be NULL.");

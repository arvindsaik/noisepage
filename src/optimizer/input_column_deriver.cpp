...
// TODO(arvindsaik): stop registering so many pointers for cleanup
void InputColumnDeriver::Visit(const Analyze *op) {
  ...
  for (const auto &col_oid : op->GetColumns()) {
    ...
    // TODO(preetang): We don't know the table name, does it matter?
    const auto &expr =
        owned_col_exprs->emplace_back(std::make_unique<parser::ColumnValueExpression>(
            "", col.Name(), db_oid, table_oid, col_oid, col.Type()));
